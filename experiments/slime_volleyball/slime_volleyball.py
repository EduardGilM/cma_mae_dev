...
def show_interactive_archive(archive_path, seed):
    ...
    Note:
        measure_0(impact_x_pos) assumed to be in range [0.0, 200.0]
        measure_1(impact_y_vel) assumed to be in range [0.0, 50.0]
        archive resolution assumed to be 50x50. 
    ...
    def onclick(event):
        ...
        print(
            f" behavior_0(num_hits):{matching_row.behavior_0.iloc[0]} \n behavior_1(num_volleys):{matching_row.behavior_1.iloc[0]} \n objective:{restored_obj}"
        )
...
if __name__ == '__main__':
    ...
    #   left click on the heatmap to play a policy with the corresponding measures
    # show_interactive_archive("logs/cma_mae_50_0.01_0.0/trial_0/archive_00002500.pkl", 14)
