...
def simulate(model, seed=None, video_env=None, save_video_to=None):
    ...
    Returns:
        obj (float): The remaining points of the right player minus the remaining
            points of the left player at the end of the game.
        num_hits (int): The number of times the right player hits the ball.
    ...
    policy = Model(mlp.games['slimevolley'])
    policy.set_model_params(model)
    ...
    while not done:
        action_right, action_left = policy.predict(obs_right), policy.predict(obs_left)
    ...
    return obj, num_hits, num_volleys
...
def simulate_parallel(client, sols, seed):
    ...
    # Process the results.
    for obj, num_hits, num_volleys in results:
        objs.append(obj)
...
def slime_volleyball_main(algorithm,
    ...
    if arch_res_exp:
        print(f"Running arch res experiment on slime volleyball and alpha={alpha}.")
    else:
        print(f"Running slime volleyball, alpha={alpha}, and resolution={resolution}.")
...
def play_policy(archive_path, max_mea, qcut_quantile, seed, outdir=None):
    ...
    ax.add_patch(Ellipse(patch_coords, width=unit_cell_size[0]*7.5, height=unit_cell_size[1]*7.5, color='red', fill=False))
    ax.add_patch(
        Rectangle(
